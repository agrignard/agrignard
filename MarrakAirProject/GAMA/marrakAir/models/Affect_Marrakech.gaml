...
//************************************************************************************************************************************************************
	...
	string node_shape <- "../includes/SIG_demonstrateur/nodes_gama.shp";
	string PM <- "../includes/SIG_demonstrateur/PM_T.shp";
...
//************************************************************************************************************************************************************	
	...
	graph roads_graph;
	geometry shape <- envelope(mynetwork);
	...
	map<float,list<list<float>>> readCopertData(string fileName)
	{
		...
		matrix<float> copert <- matrix<string>(csv_file( fileName, ';'));
...
grid pollutant_grid width:30 height:30//width:50 height:50
	{
	
		float maxNox <- 1 update: max(pollutant_grid collect(each.pollutant[world.pollutentIndex("nox")]));
...
species car skills: [moving]
{
	...
	reflex cloud
	{
		...
		pollutant_grid  cells <- first(pollutant_grid overlapping self);
...
experiment affect type:gui
{
	...
	output {
		...
		display Suivi_Vehicules  refresh_every:15
		{
			//grid parcArea;
